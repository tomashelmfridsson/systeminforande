...
Docker/Hugging Face-deployen använder Agentic RAG som standard med `SYSTEMINFORANDE_ENABLE_AGENTIC_RAG=true`. Gradio använder miljöns standardvärde om URL:en inte överstyr det. Den äldre kontrollerade RAG-vägen finns kvar för rollback och jämförelse och kan väljas tillfälligt med `?enable_agentic_rag=false` på GUI-adressen. Ett `/api/ask`-anrop kan på motsvarande sätt välja kontrollvägen med `"enable_agentic_rag": false` eller agentvägen med `"enable_agentic_rag": true`, vilket gör jämförelsen reproducerbar på samma revision. Om samma miljövariabel finns bland Space-inställningarna i Hugging Face måste den vara `true` eller tas bort, eftersom Space-värdet överstyr Docker-standardvärdet.
